...
const Dashboard = () => {
    ...
    useEffect(() => {
        if (!loading && !error && data && !data.getHotel) {
            setHotelModal({
                state: true,
    ...
    }, [loading, error, data, setHotelModal]);
    ...
    return (
        <PageContainer>
            ...
            <QuickView>
                <Info style={{ backgroundImage: `url(${(hotel && hotel.image) ? hotel.image : HotelIMG})` }}>
                    {hotel ? (
                        <>
                            ...
                            <Controls>
                                {controls.map(c => (
                                    <div key={c.label} className="card-option" onClick={c.action}>
                                        {c.icon}
                                        <p>{c.label}</p>
                                    </div>
                                ))}
                        ...
                        <div className="card-details">
                            <h1>No Hotel Information</h1>
                            <p>Please create or assign a hotel.</p>
                        </div>
                    )}
                </Info>

                <RoomsList rooms={hotel ? hotel.rooms : []} setRoomModal={setRoomModal} />
            </QuickView>
